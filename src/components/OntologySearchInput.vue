<template>
  <div class="relative w-80 mx-auto mt-4 mb-8">
    <search-focus @keyup="focusSearch"></search-focus>

    <div class="relative w-75">
      <input
              type="text"
              :placeholder="`Search (v${this.version})`"
              class="bg-background-form border border-gray-500 rounded-full px-4 pl-10 py-2 mt-0 mb-4 outline-none focus:border-customcolors-blue0 w-80"
              v-model="query"
              @input="softReset"
              @keyup="performSearch"
              @keyup.esc="searchResultsVisible = true"
    ...
    data() {
      return {
        ...
        options: {
          ...
          keys: ['generatedLabel', 'definitions', 'generatedAlias']
        }
      }
    },
    methods: {
      ...
      performSearch() {
        this.$search(this.query, this.tagsets, this.options).then(results => {
